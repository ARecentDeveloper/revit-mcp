...
export function registerElementFilterTool(server: McpServer) {
  server.tool(
    "element_filter",
    "An intelligent Revit element querying tool with optimized parameter extraction and tabular response format for maximum efficiency. PARAMETER RESOLUTION WORKFLOW: ALWAYS call resolve_parameter_names FIRST when users mention parameter names that aren't exact Revit parameter names (e.g., 'grid', 'spacing', 'thickness') - this ensures correct parameter resolution especially for curtain walls and category-specific parameters. Skip resolve_parameter_names ONLY when using ElementId filters or requesting basic element info without specific parameters. TABULAR FORMAT: Responses are optimized by grouping elements by parameter values instead of repeating data - saves 60-80% tokens! Elements with same values are grouped together for efficient processing. CRITICAL: When using parameterFilters, you MUST specify filterCategory - parameter mapping requires knowing the element category to work properly. Without filterCategory, you'll get an error. BATCHING: For multiple elements, use arrays in parameter values to query them in ONE call instead of multiple calls - much more efficient! Example: {filterCategory: 'OST_StructuralFraming', parameterFilters: [{name: 'ElementId', value: [275260, 275283, 275284], operator: 'equals'}]} queries 3 elements at once. EFFICIENCY TIP: When you have specific element IDs, query them directly using parameterFilters with ElementId rather than using broad category filters - this is much faster and more token-efficient. IMPORTANT: When users ask for specific parameters (like 'flange thickness', 'number of studs', 'web thickness'), always use the 'requestedParameters' field to explicitly request those parameters - they won't appear automatically. PARAMETER VALUES: For numeric parameters, RawValue contains precise values in Revit internal units - dimensional parameters (lengths, widths, thicknesses, heights) are in feet, areas in square feet, volumes in cubic feet, angles in radians, counts/integers as-is. Always use RawValue for accurate conversions, never parse display strings. CURTAIN WALL SYSTEM: CRITICAL - Use OST_Walls for BOTH regular walls AND curtain walls (system automatically detects curtain walls using Wall.CurtainGrid property and applies specialized curtain wall parameter mappings with parameters like 'vertical spacing', 'horizontal angle', 'curtain panel', etc.). Use OST_CurtaSystem for curtain systems (different category with 'grid 1', 'grid 2' parameters). Use OST_CurtainWallPanels for curtain panels, OST_CurtainWallMullions for mullions. Examples: 1) User asks 'What's the flange thickness of this beam?' → FIRST call resolve_parameter_names with ['flange thickness'], THEN use: {filterCategory: 'OST_StructuralFraming', requestedParameters: [resolved_name], parameterFilters: [{name: 'ElementId', value: 'ID', operator: 'equals'}]}. 2) User asks 'Find beams with flange thickness > 0.2' → FIRST resolve 'flange thickness', THEN use: {filterCategory: 'OST_StructuralFraming', parameterFilters: [{name: resolved_name, operator: 'greater', value: 0.2}]}. 3) User asks 'Find curtain walls with vertical spacing > 6 feet' → FIRST resolve 'vertical spacing' with elementId for context, THEN use: {filterCategory: 'OST_Walls', parameterFilters: [{name: resolved_name, operator: 'greater', value: 6}]}. 4) User asks 'Tell me about this element' → Use: {detailLevel: 'basic'} for minimal info (no resolution needed). 5) BATCHING: Multiple elements → Use: {filterCategory: 'OST_StructuralFraming', parameterFilters: [{name: 'ElementId', value: [275260, 275283, 275284], operator: 'equals'}]} (no resolution needed for ElementId).",
    {
      data: z.object({
        filterCategory: z
          .string()
          .optional()
          .describe("REQUIRED when using parameterFilters! Enumeration of built-in element categories in Revit used for filtering and identifying specific element types (e.g., OST_Walls, OST_Floors, OST_StructuralFraming, OST_GenericModel). Parameter mapping needs the category to resolve parameter names correctly. Common categories: OST_StructuralFraming (beams/columns), OST_Walls, OST_Floors, OST_Doors, OST_Windows, OST_Furniture, OST_GenericModel. CURTAIN WALL CATEGORIES: Use OST_Walls for BOTH regular walls AND curtain walls (system auto-detects curtain walls and applies specialized parameter mappings), OST_CurtaSystem for curtain systems, OST_CurtainWallPanels for curtain panels, OST_CurtainWallMullions for mullions."),
        filterElementType: z
          .string()
          .optional()

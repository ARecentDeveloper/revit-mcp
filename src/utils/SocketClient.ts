...
export class RevitClientConnection {
  ...
  private processBuffer(): void {
    try {
      ...
      // If successfully parsed, process response and clear buffer
      this.handleResponse(this.buffer);
      this.buffer = "";
    } catch (e) {
      // If parsing fails, may be due to incomplete data, continue waiting for more data
    }
  }
  ...
  public sendCommand(command: string, params: any = {}): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Create JSON-RPC standard compliant request object
        const commandObj = {
          jsonrpc: "2.0",
          method: command,
          params: params,
